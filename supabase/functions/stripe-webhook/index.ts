...
// Verifies the Stripe signature, then upserts the buyer's access code so the
// app can unlock profile creation. The code is generated by the app and sent
// to Stripe via `client_reference_id`. Disable JWT verification on this
// function — Stripe doesn't send a JWT, only its own signature header.
